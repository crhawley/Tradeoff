---
title: "FinalBayesianTradeoffForPublication"
author: "Caitlin"
date: "10/25/2021"
output: pdf_document
---

SetWD, Load Libraries, Set Them
```{r setup, include=FALSE}

#remove previous workspace, setwd
#rm(list=ls())
setwd("G:/My Drive/Graduate/MASTERS/Data Files/DATA FRAMES FOR USE")
...
Determine residual deviance:df ratio

```{r pressure, echo=FALSE}
RD_df_ratio <- glm(a1fs ~ nprimassoc + nprimassoc + n_fs+ grp+ offset(log(obshours)), data = Part3, family="poisson")
summary(RD_df_ratio)
#4833.0  on 2006  degrees of freedom
```


Model and Diagnostics~~~~Model 1 
```{r pressure, echo=FALSE}
Part3_zinb1 <- brm(a1fs ~ nprimassoc + nprimassoc + n_fs + grp + offset(log(obshours)) + (1|male),
                       ...
                       prior=c(set_prior("normal(0,2)", class = "b", coef="nprimassoc"),
                               ...
                               set_prior("normal(0,2)", class = "b", coef="grpPHG"),
...
P.Part3.low<- fitted(Part3_zinb1, newdata = new.d.Part3.low,  re_formula = NA,   scale = c("response"),nsamples=1000,summary=FALSE,offset=TRUE)
...
P.Part3.mid<- fitted(Part3_zinb1, newdata = new.d.Part3.mid,  re_formula = NA,   scale = c("response"),nsamples=1000,summary=FALSE,offset=TRUE)
...
P.Part3.high<- fitted(Part3_zinb1, newdata = new.d.Part3.high,  re_formula = NA,   scale = c("response"),nsamples=1000,summary=FALSE,offset=TRUE)
...
Part3.ManyPAs<-P.Part3.low %>%
  ...
  Part3theme+
  coord_cartesian(xlim=c(.5,8), ylim=c(0,5))+
  annotate("point", x=12.8, y=.8, color="red4", size=6, alpha=.3)+
  annotate("text", x = 7, y = .8, label = "2+ Primary Associations", color = "black", size=8, fontface="bold") +
...
```{r pressure, echo=FALSE}
Part4_binom1 <- brm(appdummy | trials(3357) ~ PACat + Z.Rank + grp + (1 | male) + (1 | partner) + (1|dyad_ID) + (1|date),
...
P.Part4<-fitted(Part4_binom1, newdata = new.d.Part4,  re_formula = NA,   scale = c("response"),nsamples=1000,summary=FALSE,offset=TRUE)
